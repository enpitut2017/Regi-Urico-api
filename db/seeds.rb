...
tc1 = s1.items.create!(name: '東方魔烈槍')
zc1 = s1.items.create!(name: 'かいけつゾロリのたのしー！どうぶつえん')
kbc1 = s1.items.create!(name: '野生解放！火神くん！')
kbc2 = s4.items.create!(name: '城凛ドキドキクリスマス')
opc1 = s4.items.create!(name: '正義は勝つって!? そりゃそうだろ ドン勝だけが正義だ！')
opa1 = s4.items.create!(name: 'ハンコックモチーフポチ袋')
hrakc1 = s1.items.create!(name: 'それでもお前は俺の嫁（勝デク♀）')
hrakc2 = s8.items.create!(name: '1番の理解者（出勝）（全年齢）')
ygc1 = s8.items.create!(name: 'こんなドローはいやだ！（4コマ）')

# item_id: 1-17
yga1 = s8.items.create!(name: '赤遊イラストつきトートバッグ（ARC-V）')
nc1 = s4.items.create!(name: '曲げない欲求')
nc2 = s4.items.create!(name: '追憶の里')
na1 = s4.items.create!(name: 'ナルサス ポストカードセット')
sgc1 = s8.items.create!(name: '交差する剣（リヴァエレ）')
sgc2 = s8.items.create!(name: '巨人フレンズ（クロスオーバー）')
sgg1 = s8.items.create!(name: 'シンゲキブレイド ベータ版')
...
chrakc2 = comcity.event_items.create!(price: 700, item: hrakc1)
